...
from libs.pocstrike.lib.core.common import check_port
from libs.pocstrike.lib.core.common import get_host_ip, get_host_ipv6
...
class PHTTPSingleton(type):
    '''
    HTTP server only allow one instance in pocstrike
    '''
    _instance = None
...
class PHTTPServer(threading.Thread, metaclass=PHTTPSingleton):
    def __init__(self, bind_ip='0.0.0.0', bind_port=666, is_ipv6=False, use_https=False,
                 certfile=os.path.join(paths.POCSTRIKE_DATA_PATH, 'cacert.pem'),
        ...
        if ':' in bind_ip:
            ...
            if not ipv6:
                ...
                raise PocstrikeSystemException
            self.host_ip = ipv6
    ...
    def start(self, daemon=True):
        # Http server can only allow start once in pocstrike, avoid muti-threading start muti-times
