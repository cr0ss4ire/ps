...
from libs.pocstrike.lib.core.data import logger, paths
from libs.pocstrike.lib.core.enums import CUSTOM_LOGGING, OS, OS_ARCH, SHELLCODE_CONNECTION
# for pocstrike 2.x
from libs.pocstrike.lib.core.exception import PocstrikeGenericException
from libs.pocstrike.lib.core.register import register_poc as register
from libs.pocstrike.shellcodes import OSShellcodes
...
def generate_shellcode_list(listener_ip, listener_port, os_target=OS.WINDOWS, os_target_arch=OS_ARCH.X86,
    ...
    filepath = os.path.join(paths.POCSTRIKE_TMP_PATH, filename)
    if os_target == OS.WINDOWS:
        filepath = os.path.join(paths.POCSTRIKE_TMP_PATH, filename) + '.exe'
    ...
    if not os.path.exists(filepath):
        raise PocstrikeGenericException("generate file does not exist!")
