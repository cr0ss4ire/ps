...
from libs.pocstrike.lib.core.common import parse_target_url, desensitization
from libs.pocstrike.lib.core.data import conf
from libs.pocstrike.lib.core.data import logger
from libs.pocstrike.lib.core.enums import OUTPUT_STATUS, CUSTOM_LOGGING, ERROR_TYPE_ID, POC_CATEGORY
from libs.pocstrike.lib.core.exception import PocstrikeValidationException
from libs.pocstrike.lib.core.interpreter_option import OptString, OptInteger, OptIP, OptPort, OptBool
from libs.pocstrike.lib.utils import str_to_dict

# from libs.pocstrike.lib.core.register import register_poc as register
...
class POCBase(object):
    ...
    def get_option(self, name):
        if name not in self.options:
            raise PocstrikeValidationException
    ...
    def set_option(self, key, value):
        ...
        if key not in self.options:
            raise PocstrikeValidationException("No key " + key)
    ...
    def execute(self, target, headers=None, params=None, mode='verify', verbose=True):
        ...
        try:
            ...
            msg = 'POC-{0}: {1} not defined "{2}" mode'.format(self.vulID, self.name, self.mode)
            self.expt = (CUSTOM_LOGGING.ERROR, msg)
            output = Output(self)

        except ConnectTimeout as e:
            while conf.retry > 0:
                logger.debug('POC-{0}: {1} timeout, start it over.'.format(self.vulID, self.name))
                try:
                    output = self._execute()
                    break
                except ConnectTimeout:
                    logger.debug('POC-{0}: {1} time-out retry failed!'.format(self.vulID, self.name))
                conf.retry -= 1
            else:
                msg = "{{{0}}} target connect failed!".format(desensitization(target) if conf.ppt else target)
                self.expt = (CUSTOM_LOGGING.ERROR, msg)
                output = Output(self)

        except HTTPError as e:
            msg = 'POC-{0}: {1} HTTPError occurs, start it over.'.format(self.vulID, self.name)
            self.expt = (CUSTOM_LOGGING.ERROR, msg)
            output = Output(self)

        except AttributeError as e:
            msg = 'POC-{0}: {1} AttributeError occurs, {2}'.format(self.vulID, self.name, str(e))
            self.expt = (CUSTOM_LOGGING.ERROR, msg)
            output = Output(self)
            ...
            self.expt = (CUSTOM_LOGGING.ERROR, e)
            output = Output(self)
        ...
        if isinstance(output, Output):
            ...
            self.expt = (CUSTOM_LOGGING.ERROR, "Poc-{0} _{1} function not return Output".format(self.vulID, self.mode))
            output = Output(self)
        return output
...
class Output(object):
    ...
    def error(self, error=""):
        ...
        self.msg = (CUSTOM_LOGGING.ERROR, error)

    def info(self, info=""):
        # self.expt = (ERROR_TYPE_ID.OTHER, info)
        self.msg = (CUSTOM_LOGGING.SYSINFO, info)
    ...
    def show_result(self):
        ...
        elif self.status == OUTPUT_STATUS.FAILED:
            logger.log(int(self.msg[0]), self.msg[1])
