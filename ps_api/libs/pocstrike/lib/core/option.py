...
from libs.pocstrike.lib.core.clear import remove_extra_log_message
from libs.pocstrike.lib.core.common import boldify_message, check_file, get_file_items, parse_target, \
    get_public_type_members, data_to_stdout
from libs.pocstrike.lib.core.common import check_path, extract_cookies
from libs.pocstrike.lib.core.common import get_local_ip, desensitization
from libs.pocstrike.lib.core.common import single_time_warn_message
...
from libs.pocstrike.lib.core.data import merged_options
from libs.pocstrike.lib.core.data import paths
from libs.pocstrike.lib.core.datatype import AttribDict
from libs.pocstrike.lib.core.enums import HTTP_HEADER, CUSTOM_LOGGING, PROXY_TYPE
from libs.pocstrike.lib.core.exception import PocstrikeSyntaxException, PocstrikeSystemException, PocstrikeHeaderTypeException
from libs.pocstrike.lib.core.log import FORMATTER
from libs.pocstrike.lib.core.register import load_file_to_module
from libs.pocstrike.lib.core.settings import DEFAULT_USER_AGENT, DEFAULT_LISTENER_PORT, CMD_PARSE_WHITELIST
from libs.pocstrike.lib.parse.cmd import DIY_OPTIONS
from libs.pocstrike.lib.parse.configfile import config_file_parser
from libs.pocstrike.lib.request.patch import patch_all
from libs.pocstrike.modules.listener import start_listener
from libs.pocstrike.thirdparty.oset.orderedset import OrderedSet
from libs.pocstrike.thirdparty.pysocks import socks


def _resolve_cross_references():
    import pocstrike
    pocstrike.lib.core.revision.stdout_encode = stdout_encode
    pocstrike.lib.core.convert.single_time_warn_message = single_time_warn_message


def set_verbosity():
    """
    This function set the verbosity of pocstrike output messages.
    """
...
def _set_http_user_agent():
    ...
    if conf.random_agent:
        uapath = os.path.join(paths.POCSTRIKE_DATA_PATH, 'user-agents.txt')
...
def _set_network_timeout():
    if conf.timeout:
        ...
        if conf.timeout < 3.0:
            warn_msg = "the minimum HTTP timeout is 3 seconds, pocstrike "
...
def _set_network_proxy():
    if conf.proxy:
        ...
        try:
            ...
            raise PocstrikeSyntaxException(err_msg)
...
def _set_connect_back():
    ...
    if conf.mode == "shell" and conf.connect_back_host is None:
        data_to_stdout("[i] pocstrike is running in shell mode, you need to set connect back host:\n")
        ...
        for poc in conf.poc:
            ...
            '''if any([poc in exists_poc_with_ext, poc in exists_pocs]):
                ...
                if poc_ext in ['.py', '.pyc']:
                    ...
                    file_path = os.path.join(paths.POCSTRIKE_POCS_PATH, poc + exists_pocs.get(poc))
                if file_path:
...
def _set_plugins():
    ...
    if conf.plugins:
        ...
        for found in glob.glob(os.path.join(paths.POCSTRIKE_PLUGINS_PATH, "*.py*")):
...
def _cleanup_options():
    ...
    if conf.cookie:
        if isinstance(conf.cookie, str):
            ...
            raise PocstrikeHeaderTypeException('Does not support type for cookie')
    if conf.delay:
...
def _create_directory():
    if not os.path.isdir(paths.POCSTRIKE_OUTPUT_PATH):
        try:
            os.makedirs(paths.POCSTRIKE_OUTPUT_PATH)
        except FileExistsError:
            pass

    if not os.path.isdir(paths.POCSTRIKE_TMP_PATH):
        try:
            os.makedirs(paths.POCSTRIKE_TMP_PATH)
        except FileExistsError:
            pass

    if not os.path.isfile(paths.POCSTRIKE_RC_PATH):
        open(paths.POCSTRIKE_RC_PATH, 'a').close()
