...
from libs.pocstrike.lib.core.exception import PocstrikeConnectionException
from libs.pocstrike.lib.core.exception import PocstrikeThreadException
from libs.pocstrike.lib.core.exception import PocstrikeUserQuitException
from libs.pocstrike.lib.core.exception import PocstrikeValueException
from libs.pocstrike.lib.core.settings import MAX_NUMBER_OF_THREADS
...
def run_threads(num_threads, thread_function, args: tuple = (), forward_exception=True, start_msg=True):
    ...
    except (KeyboardInterrupt, PocstrikeUserQuitException) as ex:
        kb.thread_continue = False
        kb.thread_exception = True
    ...
    except (PocstrikeConnectionException, PocstrikeValueException) as ex:
        kb.thread_exception = True
        logger.error("thread {0}: {1}".format(threading.currentThread().getName(), str(ex)))
        if conf.verbose > 1:
            traceback.print_exc()
