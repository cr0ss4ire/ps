...
from libs.pocstrike.lib.core.common import banner, index_modules, data_to_stdout, humanize_path, module_required, \
    get_poc_name, stop_after, get_local_ip, is_ipv6_address_format, rtrim, ltrim, exec_cmd
from libs.pocstrike.lib.core.data import logger, paths, kb, conf
from libs.pocstrike.lib.core.enums import POC_CATEGORY, AUTOCOMPLETE_TYPE
from libs.pocstrike.lib.core.exception import PocstrikeBaseException, PocstrikeShellQuitException
from libs.pocstrike.lib.core.option import _set_listener, _set_http_referer, _set_http_user_agent, _set_network_proxy, \
    _set_network_timeout
from libs.pocstrike.lib.core.register import load_file_to_module
from libs.pocstrike.lib.core.settings import IS_WIN
from libs.pocstrike.lib.core.shell import auto_completion, readline
from libs.pocstrike.thirdparty.prettytable import prettytable
...
class BaseInterpreter(object):
    ...
    def get_command_handler(self, command):
        ...
        try:
            ...
            raise PocstrikeBaseException("pocstrike Unknown this command, and run it on system: '{}'".format(command))
    ...
    def start(self):
        ...
        while True:
            ...
            except PocstrikeBaseException as warn:
                logger.warn(warn)
            except EOFError:
                logger.info("Pocstrike stopped")
                break
...
class PocstrikeInterpreter(BaseInterpreter):
    ...
    def __init__(self):
        super(PocstrikeInterpreter, self).__init__()

        self.current_module = None
        self.raw_prompt_template = None
        self.module_prompt_template = None
        self.prompt_hostname = "pocstrike"
        ...
        for module in self.modules:
            ...
            if IS_WIN:
                ...
                temp_module = temp_module.replace(paths.POCSTRIKE_ROOT_PATH, "").lstrip("\\")
            temp_module = temp_module.replace(paths.POCSTRIKE_ROOT_PATH, "").lstrip("/")
    ...
    def module_metadata(self):
        return getattr(self.current_module, "pocstrike_module_path")
    ...
    def command_use(self, module_path, *args, **kwargs):
        ...
        if not os.path.exists(module_path):
            module_path = os.path.join(paths.POCSTRIKE_ROOT_PATH, module_path)
            if not os.path.exists(module_path):
        ...
        try:
            ...
            self.current_module.pocstrike_module_path = ltrim(rtrim(module_path, ".py"),
                                                              os.path.join(paths.POCSTRIKE_ROOT_PATH, ""))
    ...
    def command_list(self, *args, **kwargs):
        ...
        for tmp_module in self.main_modules_dirs:
            found = os.path.join(paths.POCSTRIKE_ROOT_PATH, tmp_module + ".py")
