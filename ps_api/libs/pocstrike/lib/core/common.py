...
from libs.pocstrike.lib.core.convert import stdout_encode
from libs.pocstrike.lib.core.data import conf
from libs.pocstrike.lib.core.data import kb
from libs.pocstrike.lib.core.data import logger
from libs.pocstrike.lib.core.data import paths
from libs.pocstrike.lib.core.decorators import cachedmethod
from libs.pocstrike.lib.core.enums import OS_ARCH, OS
from libs.pocstrike.lib.core.exception import PocstrikeSystemException
from libs.pocstrike.lib.core.log import LOGGER_HANDLER
from libs.pocstrike.lib.core.settings import (
...
from libs.pocstrike.lib.core.settings import IPV6_ADDRESS_REGEX
from libs.pocstrike.lib.core.settings import IP_ADDRESS_REGEX
from libs.pocstrike.lib.core.settings import OLD_VERSION_CHARACTER
from libs.pocstrike.lib.core.settings import POCSTRIKE_VERSION_CHARACTER
from libs.pocstrike.lib.core.settings import POC_NAME_REGEX
from libs.pocstrike.lib.core.settings import POC_REQUIRES_REGEX
from libs.pocstrike.lib.core.settings import UNICODE_ENCODING
from libs.pocstrike.lib.core.settings import URL_ADDRESS_REGEX
from libs.pocstrike.thirdparty.colorama.initialise import init as coloramainit
from libs.pocstrike.thirdparty.ifcfg import ifcfg
from libs.pocstrike.thirdparty.termcolor.termcolor import colored
...
def check_file(filename):
    ...
    if not valid:
        raise PocstrikeSystemException("unable to read file '%s'" % filename)
    return valid
...
def set_paths(root_path):
    ...
    """
    paths.POCSTRIKE_ROOT_PATH = root_path
    paths.POCSTRIKE_DATA_PATH = os.path.join(paths.POCSTRIKE_ROOT_PATH, "data")
    paths.POCSTRIKE_PLUGINS_PATH = os.path.join(paths.POCSTRIKE_ROOT_PATH, "plugins")
    paths.POCSTRIKE_POCS_PATH = os.path.join(paths.POCSTRIKE_ROOT_PATH, "pocs")
    paths.USER_POCS_PATH = None

    paths.USER_AGENTS = os.path.join(paths.POCSTRIKE_DATA_PATH, "user-agents.txt")
    paths.WEAK_PASS = os.path.join(paths.POCSTRIKE_DATA_PATH, "password-top100.txt")
    paths.LARGE_WEAK_PASS = os.path.join(paths.POCSTRIKE_DATA_PATH, "password-top1000.txt")

    paths.POCSTRIKE_HOME_PATH = os.path.expanduser("~")
    _ = os.path.join(paths.POCSTRIKE_HOME_PATH, ".pocstrike")
    ...
    paths.POCSTRIKE_SHELL_HISTORY = os.path.join(_, "pocstrike.hst")
    paths.POCSTRIKE_CONSOLE_HISTORY = os.path.join(_, "console.hst")

    paths.POCSTRIKE_TMP_PATH = os.path.join(_, "tmp")
    paths.POCSTRIKE_RC_PATH = os.path.join(paths.POCSTRIKE_HOME_PATH, ".pocstrikerc")
    paths.POCSTRIKE_OUTPUT_PATH = paths.get("POCSTRIKE_OUTPUT_PATH", os.path.join(_, "output"))
    paths.SHELLCODES_DEV_PATH = os.path.join(paths.POCSTRIKE_ROOT_PATH, "shellcodes", "tools")


def banner():
    """
    Function prints pocstrike banner with its version
    """
    _ = BANNER
...
def get_latest_revision():
    ...
    ret = None
    resp = requests.get(url="https://test.com")
    try:
...
def is_pocstrike_poc(poc_string):
    for _ in POCSTRIKE_VERSION_CHARACTER:
        if _ in poc_string:
            return True
    return False
...
def is_pocstrike_poc(poc_string):
    return True if "pocstrike" in poc_string else False
...
def get_file_items(filename, comment_prefix='#', unicode_=True, lowercase=False, unique=False):
    ...
    try:
        ...
        raise PocstrikeSystemException(err_msg)
...
def write_file(data, file_ext='', file_name=''):
    ...
    if not file_name:
        file_name = TIMESTAMP
    file_name += file_ext
    file_path = os.path.join(paths.POCSTRIKE_TMP_PATH, file_name)
...
def index_modules() -> list:
    ...
    modules = []
    for root, dirs, files in os.walk(paths.POCSTRIKE_POCS_PATH):
        _, package, root = root.rpartition("pocstrike/pocs/".replace("/", os.sep))
